...
- There is at most one active saved calculation per business date. Any allowed staff member may create the first save for the current Moscow business day; later saves for that day are blocked.
- Only the configured owner Telegram ID may replace the current day's saved calculation, and only while that same Moscow business date is still current (before 00:00 Europe/Moscow). Replaced rows remain only as an audit trail and must not affect normal history or balances.
